...
# Modified parts are noticed with comments (e.g., "added/changed by Goro Kobayashi").
...
class BertSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, output_norms=False): # changed by Goro Kobayashi
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        pre_ln_states = hidden_states + input_tensor # added by Goro Kobayashi
        post_ln_states = self.LayerNorm(pre_ln_states) # changed by Goro Kobayashi
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(
            hidden_states, attention_mask, head_mask, output_attentions=output_attentions, output_norms=output_norms,
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        return outputs  # last-layer hidden state, (all hidden states), (all attentions), (all norms)
        #--------------------
...
class BertModel(BertPreTrainedModel):
    ...
    Return:
        ...
        outputs = (sequence_output, pooled_output,) + encoder_outputs[
            1:
        ]  # add hidden_states, attentions, and norms if they are here
        return outputs  # sequence_output, pooled_output, (hidden_states), (attentions), (norms)
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        output_attentions=None,
        output_hidden_states=None,
        output_norms=None, #added by the Author
    ):
        r"""
    ...
    Returns:
        ...
        outputs = self.bert(
            ...
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            output_norms=output_norms, #added by the Author
        )
